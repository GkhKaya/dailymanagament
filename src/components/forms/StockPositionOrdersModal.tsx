...
export function StockPositionOrdersModal({
  ...
  const handleDelete = async (tradeId: string) => {
    if (!confirm("Bu işlemi silmek istediğinize emin misiniz? Maliyetler geriye dönük tekrar hesaplanacaktır.")) {
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        toast.success("İşlem silindi ve portföy güncellendi.");
        onSuccess();
      } else {
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-3 sm:p-6 backdrop-blur-md bg-black/70 animate-fade-in">
      <div className="relative w-full max-w-xl max-h-[90vh] flex flex-col bg-[#12121c] rounded-3xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.8)] overflow-hidden">
        ...
        <div className="px-5 sm:px-6 py-4 flex items-center justify-between border-b border-white/10 bg-black/30 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--on-surface-variant)]">{position.name || "Borsa İstanbul"}</p>
            </div>
          </div>
        ...
        <div className="px-5 sm:px-6 py-3 bg-white/5 border-b border-white/5 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs shrink-0">
          <div>
            <span className="text-[10px] text-[var(--on-surface-variant)] uppercase block">Eldeki Lot</span>
            <span className="font-black text-white text-sm">{position.total_lots} Lot</span>
          </div>
          <div>
            <span className="text-[10px] text-[var(--on-surface-variant)] uppercase block">Ort. Maliyet</span>
            <span className="font-black text-white text-sm">
              {position.average_cost.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
            </span>
          </div>
          <div>
            <span className="text-[10px] text-[var(--on-surface-variant)] uppercase block">Toplam Maliyet</span>
            <span className="font-bold text-white text-sm">
              {position.total_cost.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
            </span>
          </div>
          <div>
            <span className="text-[10px] text-[var(--on-surface-variant)] uppercase block">Güncel Fiyat</span>
            <span className="font-bold text-white text-sm">
              {position.current_price && position.current_price > 0 ? `${position.current_price.toFixed(2)} ₺` : '—'}
            </span>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto custom-scrollbar flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-[var(--on-surface-variant)] uppercase tracking-wider">
              Giriş & Alış Emirleri ({buyTrades.length})
            </h4>
            <button
              type="button"
              onClick={() => onAddTrade('buy', position.symbol)}
              className="px-3 py-1.5 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/30 text-emerald-300 text-xs font-bold flex items-center gap-1.5 transition-all"
            >
              <Plus size={13} /> Yeni Alış Ekle
            </button>
          </div>

          {/* Buy Orders */}
          {buyTrades.length === 0 ? (
            <p className="text-xs text-[var(--on-surface-variant)] text-center py-4">Kayıtlı alış emri bulunamadı.</p>
          ) : (
              ...
              {buyTrades.map((t) => (
                ...
                >
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-black text-emerald-400">+{t.lots} Lot</span>
                      <span className="text-xs text-white/90">@ {t.price.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[var(--on-surface-variant)]">
                      <span>Tutar: <strong>{t.total_amount.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺</strong></span>
                      <span>•</span>
                      <span>{t.date}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      ...
                      className="p-2 rounded-xl bg-white/5 hover:bg-white/15 text-white/80 hover:text-white transition-colors flex items-center gap-1 text-xs font-semibold"
                      title="Giriş fiyatı veya lot adedini düzelt"
                    >
                      <Edit2 size={14} />
          ...
          {sellTrades.length > 0 && (
            <div className="flex flex-col gap-2 mt-3 pt-3 border-t border-white/5">
              <h4 className="text-xs font-bold text-[var(--on-surface-variant)] uppercase tracking-wider">
                Geçmiş Satış İşlemleri ({sellTrades.length})
              </h4>
              <div className="flex flex-col gap-2">
                {sellTrades.map((t) => (
                  <div
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-rose-400">-{t.lots} Lot</span>
                        <span>@ {t.price.toFixed(2)} ₺</span>
                        <span className={`font-bold ${t.realized_pnl && t.realized_pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                          ({t.realized_pnl && t.realized_pnl >= 0 ? '+' : ''}{t.realized_pnl?.toFixed(2)} ₺ K/Z)
                        </span>
                      </div>
