...
export function AddMealForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {activeTab === 'new' ? (
        <div className="flex flex-col gap-4 animate-fade-in">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-caption text-[var(--on-surface-variant)] uppercase tracking-wider">Yemek Ara</label>
            <div className="relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--on-surface-variant)]" size={20} />
              <input 
                type="text" 
                placeholder="Besin, marka veya yemek arayın..." 
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select className="w-full bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.1)] rounded-2xl py-4 px-4 text-body text-white focus:outline-none focus:border-[var(--inverse-primary)] transition-all appearance-none">
        ...
        <div className="flex flex-col gap-3 animate-fade-in">
          {savedRecipes.map(recipe => (
            <div key={recipe.id} className="p-4 flex items-center justify-between bg-[rgba(255,255,255,0.03)] rounded-2xl hover:bg-[rgba(255,255,255,0.05)] cursor-pointer transition-colors">
      ...
      <div className="mt-2 flex gap-3">
        ...
        <button onClick={onClose} className="flex-[2] py-4 rounded-2xl bg-[var(--inverse-primary)] hover:opacity-90 text-white font-bold transition-colors">
          Ekle
        </button>
      </div>
    </div>
