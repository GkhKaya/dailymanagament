...
import { t, getLocale, isAbroad } from '@/lib/i18n';
...
interface DBFoodResult {
  ...
  unit_type: 'gram' | 'adet' | 'piece' | 'kase' | 'bardak' | 'tabak' | 'çay kaşığı' | 'tatlı kaşığı' | 'çorba kaşığı' | 'yemek kaşığı' | string;
...
const MEAL_OPTIONS: { id: MealType; label: string; labelEn: string; icon: React.ComponentType<{ size?: number; className?: string }> }[] = [
  { id: 'breakfast', label: 'Kahvaltı', labelEn: 'Breakfast', icon: Sunrise },
  { id: 'lunch', label: 'Öğle', labelEn: 'Lunch', icon: Sun },
  { id: 'dinner', label: 'Akşam', labelEn: 'Dinner', icon: Moon },
  { id: 'snack', label: 'Ara Öğün', labelEn: 'Snack', icon: Apple }
];
...
export function AddMealForm({ onClose, onSuccess, currentDate, onOpenAIPhoto }: { onClose: () => void; onSuccess: () => void; currentDate?: string; onOpenAIPhoto?: () => void }) {
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        ...
        const abroad = isAbroad() ? '1' : '0';
        const res = await fetch(`/api/food/search?query=${encodeURIComponent(searchQuery)}&lang=${lang}&is_abroad=${abroad}`);
  ...
  const handleSelectFromDB = (food: DBFoodResult) => {
    ...
    setUnitType((food.unit_type === 'piece' ? 'adet' : food.unit_type) as any);
  ...
  const handleSelectRecent = async (food: any) => {
    ...
    if (!perUnit || (perUnit.protein_g === 0 && perUnit.carbs_g === 0 && perUnit.fat_g === 0)) {
      try {
        const lang = getLocale();
        const abroad = isAbroad() ? '1' : '0';
        const res = await fetch(`/api/food/search?query=${encodeURIComponent(food.food_name)}&lang=${lang}&is_abroad=${abroad}`);
        const data = await res.json();
    ...
    >
      ...
      {onOpenAIPhoto && (
        ...
        >
          ...
          <span>{isEn ? "Analyze & Add Food with Photo (AI)" : "Fotoğraf İle Yemek Analiz Et & Ekle (AI)"}</span>
        </button>
      )}
      ...
      <div className="flex p-1 bg-[rgba(255,255,255,0.04)] rounded-2xl">
        {(['new', 'saved'] as const).map(tab => (
          ...
          >
            {tab === 'new' ? (isEn ? 'New Meal' : 'Yeni Öğün') : (isEn ? 'Saved' : 'Kaydedilenler')}
          </button>
        ))}
      </div>
        ...
        {MEAL_OPTIONS.map(m => {
          ...
          return (
            ...
            >
              ...
              <span>{isEn ? m.labelEn : m.label}</span>
            </button>
          );
        })}
      </div>
            ...
            <div className="flex flex-col gap-2 p-3.5 bg-[rgba(74,222,128,0.06)] border border-[rgba(74,222,128,0.2)] rounded-2xl animate-fade-in">
              <div className="flex items-center justify-between px-1">
                <span className="text-[12px] font-semibold text-emerald-400 flex items-center gap-1.5">
                  <CheckCircle2 size={15} /> {isEn ? "Added this session" : "Bu Oturumda Eklenenler"} ({sessionAddedMeals.length})
                </span>
                <span className="text-[12px] font-bold text-emerald-300">
                  {isEn ? "Total:" : "Toplam:"} {sessionAddedMeals.reduce((sum, item) => sum + item.calories, 0)} kcal
                </span>
              </div>
                  ...
                  <div key={item.entry_id} className="flex items-center justify-between p-2.5 bg-[#161622] rounded-xl border border-[rgba(255,255,255,0.06)] hover:border-[rgba(255,255,255,0.12)] transition-colors">
                    <div className="flex flex-col min-w-0 pr-2">
                      ...
                      <span className="text-[11px] text-[var(--on-surface-variant)] mt-0.5">
                        {item.serving_description || `${item.quantity} ${item.unit_type}`} — <strong className="text-emerald-400">{item.calories} kcal</strong> (P:{item.protein_g}g, {isEn ? 'C' : 'K'}:{item.carbs_g}g, {isEn ? 'F' : 'Y'}:{item.fat_g}g)
                      </span>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleRemoveSessionMeal(item)}
                      className="p-1.5 text-red-400 hover:text-red-300 hover:bg-red-500/15 rounded-lg transition-colors shrink-0 cursor-pointer"
                      title={isEn ? "Remove meal" : "Öğünlerden Çıkar"}
                    >
                      <Trash2 size={15} />
                    </button>
                  </div>
            ...
            <div className="flex flex-col gap-2 relative">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder={isEn ? "Search food (e.g. Rice, Chicken, Oatmeal...)" : "Türkçe yemek arayın (Örn: Bulgur, Tavuk...)"}
                  ...
                  {searchStep === 'searching' ? (
                      ...
                      {searchResults.map(food => (
                        ...
                        >
                          <div className="flex flex-col min-w-0">
                            <div className="text-[13px] font-semibold text-white truncate flex items-center gap-1.5 flex-wrap">
                              {food.is_custom && (
                                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-300 border border-purple-500/30 shrink-0">
                                  {isAbroad() || getLocale() === 'en' ? 'Custom' : 'Özelim'}
                                </span>
                              )}
                            ...
                            <div className="text-[11px] text-[var(--on-surface-variant)] mt-0.5 flex items-center gap-2">
                              <span>
                                {food.unit_type === 'gram'
                                  ? '100g'
                                  : (food.unit_type === 'piece' || isAbroad() || getLocale() === 'en' ? '1 piece' : '1 adet')} — {Math.round((food.per_unit?.calories || 0) * (food.unit_type === 'gram' ? 100 : 1))} kcal
                              </span>
                          ...
                          <div className="flex gap-1.5 shrink-0">
                            <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium" style={{ background: 'rgba(96,165,250,0.15)', color: MACRO_COLORS.carbs }}>
                              {isAbroad() || getLocale() === 'en' ? 'C' : 'K'}{Math.round((food.per_unit?.carbs_g || 0) * (food.unit_type === 'gram' ? 100 : 1))}g
                            </span>
                            <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium" style={{ background: 'rgba(74,222,128,0.15)', color: MACRO_COLORS.protein }}>
                              P{Math.round((food.per_unit?.protein_g || 0) * (food.unit_type === 'gram' ? 100 : 1))}g
                            </span>
                            <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium" style={{ background: 'rgba(250,204,21,0.15)', color: MACRO_COLORS.fat }}>
                              {isAbroad() || getLocale() === 'en' ? 'F' : 'Y'}{Math.round((food.per_unit?.fat_g || 0) * (food.unit_type === 'gram' ? 100 : 1))}g
                            </span>
                          </div>
                        </div>
                      ))}
                      ...
                      <div className="flex border-t border-[rgba(255,255,255,0.05)]">
                        ...
                        >
                          ...
                          <span className="text-[12px] text-purple-300 font-medium">
                            {isAbroad() || getLocale() === 'en' ? 'Find with AI' : 'AI ile Bul'}
                          </span>
                        </div>
                        <div
                        ...
                        >
                          ...
                          <span className="text-[12px] font-medium">
                            {isAbroad() || getLocale() === 'en' ? 'Add Manually' : 'Manuel Ekle'}
                          </span>
                        </div>
                      </div>
                    </>
                    ...
                    <>
                      <div className="px-4 py-4 text-center text-[13px] text-[var(--on-surface-variant)]">
                        &quot;{searchQuery}&quot; {isEn ? "not found." : "bulunamadı."}
                      </div>
                      <div className="flex border-t border-[rgba(255,255,255,0.05)]">
                        <div
                          onMouseDown={(e) => { e.preventDefault(); setSearchStep('gemini_form'); setShowDropdown(false); }}
                        ...
                        >
                          <Sparkles size={16} className="text-purple-400 shrink-0" />
                          <span className="text-[12px] text-purple-300 font-medium">{isEn ? "Find with AI" : "AI ile Bul"}</span>
                        </div>
                        <div
                          onMouseDown={(e) => { e.preventDefault(); setSearchStep('manual_form'); setShowDropdown(false); }}
                          className="flex-1 px-4 py-3 flex items-center justify-center gap-2 cursor-pointer hover:bg-[rgba(255,255,255,0.08)] transition-colors text-[var(--on-surface-variant)]"
                        >
                          <PenTool size={16} className="text-emerald-400 shrink-0" />
                          <span className="text-[12px] font-medium">{isEn ? "Add Manually" : "Manuel Ekle"}</span>
                        </div>
                      </div>
                    </>
                  ) : (
                    ...
                    <div className="flex flex-col">
                      <div className="px-4 py-2 text-[10px] text-[var(--on-surface-variant)] uppercase tracking-wider font-semibold bg-[rgba(255,255,255,0.02)] border-b border-[rgba(255,255,255,0.05)]">
                        {isEn ? "Recently Added" : "Son Eklenenler"}
                      </div>
                      {(() => {
                        ...
                        if (unique.length === 0) {
                          return <div className="px-4 py-6 text-center text-[13px] text-[var(--on-surface-variant)]">{isEn ? "No meals added yet" : "Henüz eklenen yemek yok"}</div>;
                        }
              ...
              {searchStep === 'gemini_form' && (
                <div className="flex flex-col gap-3 p-4 bg-[rgba(139,92,246,0.05)] border border-[rgba(139,92,246,0.2)] rounded-2xl animate-fade-in">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[13px] font-semibold text-purple-200">{isEn ? "Find Nutrition with AI" : "AI ile Besin Değeri Bul"}</span>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-2.5 bg-purple-500 hover:bg-purple-600 text-white text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-2 cursor-pointer"
                  >
                    <Sparkles size={16} /> {isEn ? "Calculate with AI" : "AI ile Hesapla"}
                  </button>
                </div>
              )}
              ...
              {searchStep === 'manual_form' && (
                <div className="flex flex-col gap-3 p-4 bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)] rounded-2xl animate-fade-in">
                  <div className="flex items-center gap-2 mb-1">
                    <PenTool size={16} className="text-emerald-400 shrink-0" />
                    <span className="text-[13px] font-semibold text-white">{isEn ? "Add Food Manually" : "Yemeği Manuel Olarak Ekle"}</span>
                  </div>
                  ...
                  >
                    <option value="gemini">Gemini Vision ({isEn ? "Recommended" : "Önerilen"})</option>
                  ...
                  <button
                    ...
                    disabled={isOcrLoading}
                    className="min-h-[48px] w-full rounded-xl border border-[rgba(142,193,59,0.35)] bg-[rgba(142,193,59,0.08)] px-3 py-2.5 text-sm font-semibold text-[var(--primary)] transition-colors hover:bg-[rgba(142,193,59,0.14)] disabled:cursor-wait disabled:opacity-60 flex items-center justify-center gap-2 cursor-pointer"
                    aria-label={t('forms.foodLabelOcr')}
                  >
                    {isOcrLoading ? <Loader2 size={17} className="animate-spin" /> : <Camera size={17} />}
                    {isOcrLoading ? t('forms.foodLabelOcrReading') : `${ocrProvider === 'gemini' ? 'Gemini Vision' : 'Mistral OCR'} ${isEn ? 'Scan' : 'ile oku'}`}
                  </button>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <input
                      ...
                      placeholder={isEn ? "Brand (Optional)" : "Marka (İsteğe bağlı)"}
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="py-2 px-3 text-xs text-[var(--on-surface-variant)] flex items-center bg-[rgba(255,255,255,0.03)] rounded-xl border border-[rgba(255,255,255,0.05)]">
                      {isEn ? `Enter values for ${unitType === 'gram' ? '100g' : `1 ${unitType}`}` : `Değerleri ${unitType === 'gram' ? '100g' : `1 ${unitType}`} için girin`}
                    </div>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mt-1">
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-orange-400 font-semibold pl-1">{isEn ? "Calories" : "Kalori"}</label>
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-blue-400 font-semibold pl-1">{isEn ? "Carbs (g)" : "Karb (g)"}</label>
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-green-400 font-semibold pl-1">{isEn ? "Protein (g)" : "Protein (g)"}</label>
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-yellow-400 font-semibold pl-1">{isEn ? "Fat (g)" : "Yağ (g)"}</label>
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-pink-400 font-semibold pl-1">{isEn ? "Sugar (g)" : "Şeker (g)"}</label>
                  ...
                  {!isOcrReady ? (
                    <button
                      ...
                      className="w-full mt-2 py-2.5 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-black text-sm font-semibold rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                    >
                      {isEn ? "Save to DB and Select" : "Veritabanına Kaydet ve Seç"}
                    </button>
                  ) : (
                    <div className="mt-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-center text-xs text-emerald-300">
                      {isEn ? "Check the values. Click 'Add to Meal' below to save." : "Değerleri kontrol et. Kaydetmek için aşağıdaki “Öğüne Ekle” düğmesine bas."}
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="flex items-center gap-3 p-3 bg-[rgba(255,255,255,0.04)] rounded-2xl border border-[rgba(255,255,255,0.08)]">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-[var(--primary)] mt-0.5">
                    {geminiResult ? (isEn ? `AI estimate${geminiResult.provider ? `: ${geminiResult.provider}` : ''}. Check values.` : `AI tahmini${geminiResult.provider ? `: ${geminiResult.provider}` : ''}. Değerleri kontrol edin.`) : (isEn ? 'From Database (Verified)' : 'Veritabanından (Doğrulanmış)')}
                  </div>
                </div>
                <button
                  type="button"
                  onClick={handleClearSelection}
                  className="w-7 h-7 rounded-full bg-[rgba(255,255,255,0.06)] flex items-center justify-center hover:bg-[rgba(255,255,255,0.12)] transition-colors text-[var(--on-surface-variant)] cursor-pointer"
                >
                  <X size={14} />
                </button>
              </div>
                ...
                <div className="flex flex-col gap-1.5 relative" ref={portionSelectRef}>
                  <div className="flex items-center justify-between">
                    <label className="text-[11px] text-[var(--on-surface-variant)] uppercase tracking-wider font-semibold flex items-center gap-1.5">
                      ...
                      {isEn ? "Serving Unit / Portion" : "Ölçü Birimi / Porsiyon"}
                    </label>
                    {availablePortions[selectedPortionIdx] && (
                      <span className="text-[11px] text-[var(--primary)] font-medium">
                        {availablePortions[selectedPortionIdx].isRawGram
                          ? (isEn ? 'Custom Grams' : 'Serbest Gram')
                          : `${availablePortions[selectedPortionIdx].gram_weight}g / ${isEn ? 'unit' : 'birim'}`}
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full min-h-[48px] bg-[#14141E] border rounded-xl py-2.5 px-3.5 text-left flex items-center justify-between gap-2.5 transition-all outline-none cursor-pointer ${
                      isPortionMenuOpen
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <span className="text-[14px] font-bold text-white truncate">
                        {availablePortions[selectedPortionIdx]?.name || (isEn ? 'Select Portion' : 'Ölçü Seçin')}
                      </span>
                    </div>
                    <ChevronDown
                      size={17}
                  ...
                  {isPortionMenuOpen && (
                    <div className="absolute top-[calc(100%+6px)] left-0 right-0 z-50 bg-[#161624] border border-[rgba(255,255,255,0.12)] rounded-2xl shadow-2xl backdrop-blur-xl p-1.5 max-h-64 overflow-y-auto space-y-1 animate-fade-in">
                      <div className="px-2.5 py-1 text-[10px] uppercase tracking-wider font-semibold text-[var(--on-surface-variant)]">
                        {isEn ? `Available Portions (${availablePortions.length})` : `Kullanılabilir Porsiyonlar (${availablePortions.length})`}
                      </div>
                      {availablePortions.map((opt, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full px-3 py-2.5 rounded-xl text-left flex items-center justify-between gap-2 transition-all cursor-pointer ${
                              isSelected
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-[11px] text-[var(--on-surface-variant)] uppercase tracking-wider font-semibold flex items-center gap-1.5">
                      ...
                      <span>{availablePortions[selectedPortionIdx]?.isRawGram ? (isEn ? 'Amount (Grams)' : 'Miktar (Gram)') : (isEn ? 'Quantity / Multiplier' : 'Miktar / Çarpan')}</span>
                    </label>
                  ...
                  <div className="relative">
                    ...
                    <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[12px] font-bold text-[var(--on-surface-variant)] pointer-events-none uppercase">
                      {availablePortions[selectedPortionIdx]?.isRawGram ? 'g' : (isEn ? 'pcs' : 'adet')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {parseFloat(calories) > 0 && (
                <div className="p-4 bg-[rgba(255,255,255,0.03)] rounded-2xl border border-[rgba(255,255,255,0.06)] animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      { label: isEn ? 'Carbs' : 'Karb', value: nutrients.carbs, pct: nutrients.carbsPct, color: MACRO_COLORS.carbs, bg: 'rgba(96,165,250,0.1)' },
                      { label: 'Protein', value: nutrients.protein, pct: nutrients.proteinPct, color: MACRO_COLORS.protein, bg: 'rgba(74,222,128,0.1)' },
                      { label: isEn ? 'Fat' : 'Yağ', value: nutrients.fat, pct: nutrients.fatPct, color: MACRO_COLORS.fat, bg: 'rgba(250,204,21,0.1)' },
                      { label: isEn ? 'Sugar' : 'Şeker', value: nutrients.sugar, pct: 0, color: MACRO_COLORS.sugar, bg: 'rgba(244,114,182,0.1)' }
                    ].map(m => (
          ...
          {isLoadingSaved ? (
            ...
            <div className="flex flex-col items-center justify-center text-center text-[13px] text-[var(--on-surface-variant)] py-8">
              ...
              {isEn ? (
                <>
                  No saved foods yet.
                  <br />You can check &quot;Add to my favorites&quot; when adding new meals.
                </>
              ) : (
                <>
      ...
      <div className="flex gap-3 mt-1">
        <button
          type="button"
          onClick={onClose}
          className={`flex-1 py-3.5 rounded-2xl text-[13px] font-semibold transition-all cursor-pointer ${
            addedCount > 0
              ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 hover:bg-emerald-500/30'
              : 'bg-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.09)] text-white'
          }`}
        >
          {addedCount > 0 ? (isEn ? `Done (${addedCount} added)` : `Tamam (${addedCount} eklendi)`) : (isEn ? "Cancel" : "İptal")}
        </button>
        <button
          type="submit"
          disabled={
          ...
          className="flex-[2] py-3.5 rounded-2xl bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-black text-[13px] font-bold transition-colors flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
        >
          {isLoading
            ? <LoadingSpinner size="sm" />
            : activeTab === 'saved'
              ? (isEn ? `Add Selected (${selectedSavedFoods.length})` : `Seçilenleri Ekle (${selectedSavedFoods.length})`)
              : (isEn ? "Add to Meal" : "Öğüne Ekle")
          }
        </button>
      </div>
    </form>
