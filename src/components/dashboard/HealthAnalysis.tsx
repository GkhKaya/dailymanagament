...
export function HealthAnalysis({ onBack }: { onBack: () => void }) {
  ...
  const formatTimeOffset = () => {
    if (timeOffset === 0) return timeFilter === 'week' ? (isEn ? 'This Week' : 'Bu Hafta') : (isEn ? 'This Month' : 'Bu Ay');
    if (timeOffset === -1) return timeFilter === 'week' ? (isEn ? 'Last Week' : 'Geçen Hafta') : (isEn ? 'Last Month' : 'Geçen Ay');
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-white">
              {isEn ? "Detailed Health Analysis" : "Detaylı Sağlık Analizi"}
            </h2>
          </div>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto">
            <button
              ...
              title={isEn ? "Get PDF Report" : "PDF Raporu Al"}
            >
            ...
            <div className="flex bg-[rgba(255,255,255,0.05)] p-1 rounded-xl shrink-0">
              {[
                ...
                <button
                  ...
                  className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-caption font-medium transition-colors cursor-pointer ${timeFilter === f.id ? 'bg-[var(--surface-container)] text-white shadow-sm' : 'text-[var(--on-surface-variant)] hover:text-white'}`}
                >
                  {f.label}
                </button>
              ))}
      ...
      {isLoading ? (
        ...
        <div className="glass-item p-6 h-[400px] flex flex-col lg:col-span-2">
          <h3 className="text-body font-bold text-white mb-6">
            {isEn ? "Calorie Intake / Burn Trend" : "Kalori Alım / Yakım Trendi"}
          </h3>
          <div className="flex-1 w-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={calorieData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="consumed" name={isEn ? "Intake (kcal)" : "Alınan (kcal)"} stroke="#c0c1ff" strokeWidth={3} dot={{ r: 4, fill: '#c0c1ff', strokeWidth: 0 }} activeDot={{ r: 6 }} />
                <Line type="monotone" dataKey="burned" name={isEn ? "Burned (kcal)" : "Yakılan (kcal)"} stroke="#8ec13b" strokeWidth={3} dot={{ r: 4, fill: '#8ec13b', strokeWidth: 0 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-item p-6 h-[350px] flex flex-col">
          <h3 className="text-body font-bold text-white mb-6">
            {isEn ? "Sleep Duration (Hours)" : "Uyku Süreleri (Saat)"}
          </h3>
          <div className="flex-1 w-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sleepData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <div className="bg-[rgba(20,20,25,0.9)] border border-[rgba(255,255,255,0.1)] rounded-xl p-3 shadow-xl">
                          ...
                          <div className="flex flex-col gap-1">
                            <span className="text-sm text-[#818cf8]">
                              {isEn ? "Duration: " : "Süre: "}{data.sleep} {isEn ? "Hours" : "Saat"}
                            </span>
                            <span className="text-sm text-orange-400">
                              {isEn ? "Burned: " : "Yakılan: "}{data.sleepCalories} kcal
                            </span>
                          </div>
                        </div>
                      );
                ...
                />
                <Bar dataKey="sleep" name={isEn ? "Sleep Duration" : "Uyku Süresi"} fill="#818cf8" radius={[4, 4, 0, 0]} barSize={20} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-item p-6 h-[350px] flex flex-col">
          <h3 className="text-body font-bold text-white mb-2">
            {isEn ? "Macronutrient Distribution" : "Makro Besin Dağılımı"}
          </h3>
          <div className="flex-1 w-full min-h-0 relative">
            ...
            <div className="flex justify-center gap-4 mt-2">
              {macroData.map((item) => {
                const label = isEn 
                  ? (item.name === 'Karbonhidrat' ? 'Carbs' : item.name === 'Yağ' ? 'Fat' : item.name)
                  : item.name;
                return (
                  <div key={item.name} className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: item.color }}></div>
                    <span className="text-caption text-[var(--on-surface-variant)]">{label}</span>
                  </div>
                );
              })}
            </div>
