...
import { ArrowLeft, Database, Info, Activity, ShieldCheck, Search } from 'lucide-react';
      ...
      <div className="w-full max-w-3xl animate-fade-in">
        ...
        <div className="flex flex-col mb-10">
          <div className="flex items-center gap-4 mb-2">
            ...
            <h1 className="text-3xl lg:text-4xl font-extrabold text-white tracking-tight">Uygulama Hakkında</h1>
          </div>
          <p className="text-[var(--on-surface-variant)] text-sm sm:text-base ml-[56px]">
            Sürüm ve veritabanı detayları.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-[var(--surface-container-low)] border border-[var(--outline)] rounded-xl p-8 shadow-xl flex flex-col items-center text-center">
            ...
            <p className="text-[var(--on-surface-variant)] text-sm mb-6">Versiyon 1.0.0 (Beta)</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
              <div className="bg-[var(--surface-container)] rounded-xl p-4 border border-[var(--outline)] flex items-center gap-4">
                ...
                <div className="text-left flex flex-col">
                  <span className="text-sm font-bold text-white">Güvenli Altyapı</span>
                  <span className="text-xs text-[var(--on-surface-variant)]">Verileriniz güvende</span>
                </div>
              </div>
              <button 
              ...
              >
                ...
                <div className="text-left flex flex-col">
                  <span className="text-sm font-bold text-white group-hover:text-[var(--primary)] transition-colors">Veritabanımızdaki Besinler</span>
                  <span className="text-xs text-[var(--on-surface-variant)]">{foods.length} Kayıtlı Besin</span>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-2">
            {filteredFoods.length > 0 ? (
              filteredFoods.map(food => {
                // Besin porsiyon/birim bilgisini formatla
                let portionText = food.unit_type === 'gram' ? '100 gram' : '1 adet';
                if (food.nutrition_basis === 'per_unit' && food.unit_type !== 'gram') portionText = '1 adet';
                ...
                return (
                  <div key={food.id || Math.random().toString()} className="bg-[var(--surface-container)] rounded-lg p-3 border border-[rgba(255,255,255,0.04)] hover:border-[rgba(255,255,255,0.1)] transition-colors flex justify-between items-center gap-2">
                    ...
                    <div className="hidden sm:flex flex-col gap-0.5 ml-4 shrink-0 text-[10px] text-[var(--on-surface-variant)]">
                      ...
                      <span>K: <span className="text-white">{carb}g</span></span>
                      <span>Y: <span className="text-white">{fat}g</span></span>
                    </div>
                  </div>
                )
              })
