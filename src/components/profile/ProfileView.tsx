      ...
      template = [
        {
          ...
          note: isEn ? "Focus on good form and proper warm-up" : "Forma odaklan, ısınmayı iyi yap",
          exercises: [
        ...
        },
        {
          ...
          note: isEn ? "Full range of motion on pull-ups" : "Barfikste tam hareket açıklığı",
          exercises: [
                ...
                <div className="flex flex-col gap-3">
                  {workoutDays.map((day) => (
                    <WorkoutDayCard
                      key={day.id}
                      day={day}
                      isExpanded={expandedWorkoutDays.includes(day.id)}
                      onToggleExpand={() => toggleWorkoutDay(day.id)}
                      onEditDay={() => {
                        setSelectedWorkoutDay(day);
                        setActiveSheet('manageWorkoutRoutine');
                      }}
                      onRefresh={fetchWorkoutRoutine}
                      isEn={isEn}
                    />
                  ))}
                </div>
