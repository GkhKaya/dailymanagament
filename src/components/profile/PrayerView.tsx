...
import { getPrayerDataAction, getPrayerDistrictsAction, savePrayerLocationAction, savePushSubscriptionAction, scheduleTestPrayerNotificationAction } from '@/actions/prayer';
...
export function PrayerView() {
  const [province, setProvince] = useState(''); const [district, setDistrict] = useState(''); const [districts, setDistricts] = useState<string[]>([]); const [times, setTimes] = useState<PrayerDay[]>([]); const [busy, setBusy] = useState(false); const [pushReady, setPushReady] = useState(false);
  ...
  const sendTest = async () => { const result = await scheduleTestPrayerNotificationAction(); if (result.success) toast.success('Deneme bildirimi 1 dakika sonra gönderilecek.'); else toast.error(result.error || 'Deneme bildirimi kurulamadı.'); };
  ...
  return <div className="bg-[var(--surface-container-low)] border border-[var(--outline)] rounded-xl p-6 shadow-xl">
    <div className="flex items-center justify-between gap-3 mb-5"><div><h3 className="text-lg font-bold text-white flex items-center gap-2"><MapPin size={19} className="text-[var(--primary)]" /> Namaz Vakitleri</h3><p className="text-xs text-[var(--on-surface-variant)] mt-1">İl ve ilçeni seç. Aylık vakitler kaydedilsin.</p></div><div className="flex gap-2"><button onClick={sendTest} disabled={!pushReady} className="px-3 py-2 rounded-lg border border-amber-400 text-amber-300 text-xs font-bold disabled:opacity-50">1 dk test</button><button onClick={enablePush} className="px-3 py-2 rounded-lg border border-[var(--primary)] text-[var(--primary)] text-xs font-bold flex items-center gap-1.5"><Bell size={14} /> {pushReady ? 'Açık' : 'Bildirimleri aç'}</button></div></div>
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><select value={province} onChange={e => { setProvince(e.target.value); setDistrict(''); }} className="bg-[var(--surface-container)] border border-[var(--outline)] rounded-lg p-3 text-sm text-white"><option value="">İl seç</option>{TURKEY_PROVINCES.map(p => <option key={p}>{p}</option>)}</select><select value={district} onChange={e => setDistrict(e.target.value)} disabled={!province || districts.length === 0} className="bg-[var(--surface-container)] border border-[var(--outline)] rounded-lg p-3 text-sm text-white disabled:opacity-50"><option value="">{districts.length ? 'İlçe seç' : 'İl seçin'}</option>{districts.map(item => <option key={item}>{item}</option>)}</select><button onClick={save} disabled={busy || !province || !district} className="rounded-lg bg-[var(--primary)] text-black font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50"><Save size={15} /> {busy ? 'Kaydediliyor' : 'Kaydet'}</button></div>
